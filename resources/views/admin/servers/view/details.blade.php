...
@section('content-header')
    <h1>{{ $server->name }}<small>所有者やコンテナを含む、このサーバーの詳細を編集します。</small></h1>
    <ol class="breadcrumb">
            ...
            <form action="{{ route('admin.servers.view.details', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <p class="text-muted small">使用可能な文字: <code>a-zA-Z0-9_-</code> と <code>[スペース]</code>。</p>
                    </div>
                    <div class="form-group">
                        ...
                        <p class="text-muted small">このサーバーに外部識別子を割り当てない場合は空欄のままにしてください。外部IDはこのサーバーに対して一意であり、他のサーバーで使用されていない必要があります。</p>
                    </div>
                    <div class="form-group">
                        ...
                        <p class="text-muted small">このフィールドを、このシステム上の別のユーザーに一致するメールアドレスへ変更すると、サーバー所有者を変更できます。これを行うと、新しいデーモンセキュリティトークンが自動的に生成されます。</p>
                    </div>
                    <div class="form-group">
                        <label for="description" class="control-label">サーバー説明</label>
